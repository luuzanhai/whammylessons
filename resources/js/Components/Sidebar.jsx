    ...
    return (
        <aside className={`relative hidden h-screen flex-col border-r shadow-2xl transition-colors duration-300 md:flex ${isExpanded ? 'w-64' : 'w-20'} ${isDarkMode ? 'border-gray-800 bg-[#1a1a1a] text-gray-400' : 'border-gray-200 bg-[#FBFBF9] text-gray-600'}`}>
            ...
            <div className="relative mt-auto p-4">
                ...
                <div className={`flex items-center gap-2 rounded-xl p-2 ${!isExpanded && 'flex-col-reverse'} ${isDarkMode ? 'bg-[#252525]' : 'bg-[#EBEBE8]'}`}>
                    ...
                    <button type="button" aria-label="Settings" onClick={() => setIsSettingsOpen(!isSettingsOpen)} className={`flex ${isExpanded ? 'h-9 w-9' : 'h-11 w-11'} flex-shrink-0 items-center justify-center rounded-lg ${isSettingsOpen ? (isDarkMode ? 'bg-white/15 text-white' : 'bg-gray-200 text-black') : (isDarkMode ? 'text-gray-400 hover:bg-[#333] hover:text-white' : 'text-gray-600 hover:bg-gray-200 hover:text-black')}`}>
                        <svg className={isExpanded ? 'h-5 w-5' : 'h-7 w-7'} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
